...
public class BirchWriterTest
{
    ...
    public void wickedLargeTreeWithOverflow() throws Exception
    {
        ...
        try (PageAlignedReader reader = new PageAlignedReader(tmpFile))
        {
            ...
            {
                IndexInfo indexInfo = birchReader.search(sample.firstName, type, false);
    ...
    public void searchForExactMatch() throws IOException
    {
        ...
        try (PageAlignedReader reader = new PageAlignedReader(tmpFile))
        {
            ...
            IndexInfo res = birchReader.search(samples.get(3).firstName, type, false);
            Assert.assertTrue(type.compare(res.firstName, samples.get(3).firstName) == 0);
        }
    ...
    public void multipleTreesAndIterateAllElementsReversed() throws IOException
    {
        ...
        try (PageAlignedReader reader = new PageAlignedReader(tmpFile))
        {
            ...
            while (iterator.hasNext())
            {
                ...
                Assert.assertTrue(type.compare(expectedIndexInfo.firstName, next.firstName) == 0);
        ...
        try (PageAlignedReader reader = new PageAlignedReader(tmpFile))
        {
            ...
            for (IndexInfo sample : samples)
            {
                ...
                logger.debug("birchWriter returned {}:{}:{}.... looking for {}:{}:{}",
                            TimeUUIDType.instance.getString(indexInfo.firstName.toByteBuffer().duplicate()), indexInfo.offset, indexInfo.width,
                            TimeUUIDType.instance.getString(sample.firstName.toByteBuffer().duplicate()), sample.offset, sample.width);
                Assert.assertEquals(sample.offset, indexInfo.offset);
            }
        }
        ...
        try (PageAlignedReader reader = new PageAlignedReader(tmpFile))
        {
            ...
            for (int i = 0; i < 1000; i++)
            {
                for (Composite toLookup : lookup)
                {
                    ...
                    logger.info("birch returned {}:{}:{}", TimeUUIDType.instance.getString(indexInfo.firstName.toByteBuffer().duplicate()), indexInfo.offset, indexInfo.width);
                    timerContext.stop();
                }
            }
    ...
    public void simpleTreeIterateStartingFromSearchOffset() throws IOException
    {
        ...
        try (PageAlignedReader reader = new PageAlignedReader(tmpFile))
        {
            ...
            List<IndexInfo> samples = ((TimeUUIDTreeSerializableIterator) timeUUIDIterator).getSamples();
            Iterator<IndexInfo> iterator = birchReader.getIterator(samples.get(1).firstName, type, false);
            int iteratorCount = 0;
            while (iterator.hasNext())
            {
                IndexInfo next = iterator.next();

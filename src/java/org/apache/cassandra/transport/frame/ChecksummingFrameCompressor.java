...
public class ChecksummingFrameCompressor implements FrameCompressor
{
    private final FrameBodyTransformer transformer;

    public ChecksummingFrameCompressor(FrameBodyTransformer transformer)
    {
    ...
    public Frame compress(Frame frame) throws IOException
    {
        try
        {
            return frame.with(transformer.transformOutbound(frame.body));
        }
        finally
        {
    ...
    public Frame decompress(Frame frame) throws IOException
    {
        try
        {
            return frame.with(transformer.transformInbound(frame.body));
        }
        finally
        {
            //release the old frame

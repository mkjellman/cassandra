 ...
 * can be read by {@link org.apache.cassandra.io.sstable.birch.BirchReader}.
 * It is backed by a {@link org.apache.cassandra.io.sstable.birch.PageAlignedWriter}.
 ...
 * tree is made of n {@link org.apache.cassandra.io.sstable.birch.Node}
 * (see section 1.2) (leaf and inner nodes), and an encoded
 * {@link org.apache.cassandra.io.sstable.birch.Overflow} segment (optionally
 ...
 *                  {@link org.apache.cassandra.io.sstable.birch.Overflow} page.
 ...
 * {@link org.apache.cassandra.io.sstable.birch.Node} with 3 elements, a 16-byte [short]
 * value, where element 2 exceeds the max key size and has the offset for it's
 * {@link org.apache.cassandra.io.sstable.birch.Overflow} component encoded.
 * <p>
 *
 ...
 * (see {@link org.apache.cassandra.io.sstable.birch.BirchReader.BirchIterator}),
 * 2) binary search for a specific element
 * (see {@link org.apache.cassandra.io.sstable.birch.BirchReader#search(ClusteringPrefix, ClusteringComparator, boolean)}),
...
public class BirchWriter<T>
{
    ...
    private BirchWriter(Builder builder)
    {
    ...
    public void serialize(PageAlignedWriter writer) throws IOException
    {
        ...
        Node inProgressLeaf = new Node(comparator, serializer, maxKeyLengthWithoutOverflow, cacheLineSize, overflow);
        ...
        while (elementsToAddIterator.hasNext())
        {
            ...
            if (!inProgressLeaf.hasCapacity(elm))
            {
                ...
                inProgressLeaf = new Node(comparator, serializer, maxKeyLengthWithoutOverflow, cacheLineSize, overflow);
            }
        ...
        {
            Node inProgressInnerNode = new Node(comparator, serializer, maxKeyLengthWithoutOverflow, cacheLineSize, overflow);
            for (Node node : sourceLevel)
            {
                node.getElementToPromote().getTreeSerializable().serializedKey(comparator);
                ...
                if (!inProgressInnerNode.hasCapacity(node.getElementToPromote().getTreeSerializable()))
                {
                    ...
                    inProgressInnerNode = new Node(comparator, serializer, maxKeyLengthWithoutOverflow, cacheLineSize, overflow);
                }
    ...
    public static class Builder
    {
        ...
        public Builder(Iterator elms, SerializerType serializerType, ClusteringComparator comparator)
        {
            this.elms = elms;

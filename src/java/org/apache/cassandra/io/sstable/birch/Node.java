...
public class Node implements TreeSerializable
{
    ...
    public Node(ClusteringComparator comparator, BSerializer serializer, short maxKeyLength, int pageSize, Overflow overflow)
    {
    ...
    public boolean hasCapacity(TreeSerializable elm) throws IOException
    {
    ...
    public void add(WrappedTreeSerializable wrappedElm) throws IOException
    {
        add(wrappedElm.getTreeSerializable());
    }

    public void add(TreeSerializable elm) throws IOException
    {
     ...
     * {@link org.apache.cassandra.io.sstable.birch.BirchWriter} tree (see section 1.2 and 1.3).
     *
    ...
    public void serialize(PageAlignedWriter writer) throws IOException
    {
        ...
        for (int i = 0; i < elements.size(); i++)
        {
            ...
            ByteBuffer key = currentElement.getTreeSerializable().serializedKey(comparator);
    ...
    private short serializedLength(TreeSerializable elm) throws IOException
    {
        int keyLength = elm.serializedKeySize(comparator);
    ...
    {
        return getElementToPromote().getTreeSerializable().serializedKeySize(comparator);
    }

    public ByteBuffer serializedKey(ClusteringComparator comparator) throws IOException
    {
        return getElementToPromote().getTreeSerializable().serializedKey(comparator);
    }
